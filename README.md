...
| `APP_USER`           | *(unset)* | User `exec`-based commands run as in the container. Unset uses the image's own default user; `root-shell`/`root-bash` always use `root` regardless of this setting |
